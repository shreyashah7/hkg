                ...
                for (var key in $scope.deptShiftMap) {
                    ...
                    if (split[0] === e.val) {
                        angular.forEach($scope.deptShiftMap[key], function (item) {
                            angular.forEach(selectedShifts, function (col, index) {
                                if (col.id === item.value) {
...
//                        console.log("item :" + JSON.stringify(item));
            ...
            $scope.cancelSelectedValues = function () {
                ...
                $scope.transfer = {};
                $scope.shiftList = [];
//                $scope.transferEmployeeForm.selectShift.$invalid = false;
//                $scope.transferEmployeeForm.selectShift.$valid = true;
//                $scope.transferEmployeeForm.selectShift.$error.required = false;
//                $scope.transferEmployeeForm.selectDepartment.$invalid = false;
//                $scope.transferEmployeeForm.selectDepartment.$valid = true;
//                $scope.transferEmployeeForm.selectDepartment.$error.required = false;
            ...
            $scope.retrieveEmployeesByShiftAndByDept = function () {
                ...
                if ($scope.selectedDepartments && $scope.selectedShifts) {
...
//                console.log("dept :" + dept + "index " + index)
            ...
            $scope.employeeTransfer = function () {
                console.log("new employess :" + JSON.stringify($scope.newEmployeeList));
                angular.forEach($scope.newEmployeeList, function (item) {
                    if (item.selected) {
