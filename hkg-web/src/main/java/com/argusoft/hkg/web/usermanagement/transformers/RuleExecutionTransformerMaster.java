...
import com.argusoft.hkg.web.center.stock.transformers.*;
 ...
 */
@Service
public class RuleExecutionTransformerMaster {
    ...
    @Autowired
    UserManagementServiceWrapper userService;
     ...
     * @return
     */
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(RuleExecutionTransformerMaster.class);
...
//                            currentFieldValueMap1.get("username").toString(), null, Boolean.TRUE, null
                            LOGGER.info("currentFieldValueMap1 :: " + currentFieldValueMap1);
                            UMUser uMUser = userService.getUserbyUserName(currentFieldValueMap1.get("username").toString());
                            if (uMUser != null) {
                                LOGGER.info("user id" + uMUser.getId());
                                ...
                                List<Long> userOnLeave = employeeService.isUserOnLeave(uMUser.getId(), new Date(), toDate, statusList, uMUser.getCompany());
                                if (!CollectionUtils.isEmpty(userOnLeave)) {
                                    LOGGER.info("leave found");
//                                    LOGGER.info("here on leave" + "from date--" + userOnLeave.get(0).get + "to date--" + leaveDocuments.get(0).getToDt());
