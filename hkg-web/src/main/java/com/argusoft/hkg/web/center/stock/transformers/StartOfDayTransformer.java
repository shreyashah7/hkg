 ...
 */
@Service
public class StartOfDayTransformer {
...
///////////////////////////////////////////////////////////////////////////////
        ...
        BasicBSONObject issueBasicBSONObject = new BasicBSONObject();
        ArrayList<String> inStockOfDepartmentList = new ArrayList<String>();
        inStockOfDepartmentList.add(loginDataBean.getDepartment() + ":D");

        issueBasicBSONObject.put(HkSystemConstantUtil.IssueStaticFieldName.IN_STOCK_OF_DEPT, inStockOfDepartmentList);
        //System.out.println("-------issueBasicBSONObject======" + issueBasicBSONObject);
////////////////////////////////////////////////////////////////////////////////////////
        List<HkLotDocument> lotDocuments = new ArrayList<>();
        List<HkPacketDocument> packetDocuments = new ArrayList<>();

        lotDocuments = (List<HkLotDocument>) stockService.retrieveLotsOrPacketsForIssueForSod(issueBasicBSONObject, Boolean.FALSE, franchise);
        //System.out.println("-----retrieveLotsForIssueForSOD-----" + lotDocuments);
        packetDocuments = (List<HkPacketDocument>) stockService.retrieveLotsOrPacketsForIssueForSod(issueBasicBSONObject, Boolean.TRUE, franchise);
        //System.out.println("-----retrievePacketsForIssueForSOD-----" + packetDocuments);
                ...
                for (HkLotDocument lotDocument : lotDocuments) {
                    ...
                    if (!CollectionUtils.isEmpty(issueDbFieldName)) {
                        if (lotDocument.getEodIssueDocument() != null) {
                            ...
                            Map<String, String> uiFieldListWithComponentTypeForIssue = customFieldService.retrieveUIFieldNameWithComponentTypes(uiFieldListForIssue);
            ...
            if (!CollectionUtils.isEmpty(hkInvoiceDocuments) && !CollectionUtils.isEmpty(hkParcelDocuments) && !CollectionUtils.isEmpty(hklotDocuments)) {
                for (HkPacketDocument packetDocument : packetDocuments) {
                    ...
                    if (oldpacketDocument.getFieldValue().toMap().containsKey(HkSystemConstantUtil.AutoNumber.PACKET_SLIP_ID)) {
                        selectItem.setPacketSlipId(oldpacketDocument.getFieldValue().toMap().containsKey(HkSystemConstantUtil.AutoNumber.PACKET_SLIP_ID));
                    }
        ...
        //System.out.println("-----result-----from issue retrieve-----" + result.toString());
...
//        ////////////////////////////////////////////////
        ...
        //System.out.println("----lotDbFieldName dbfieldName-----" + lotDbFieldName);
        //System.out.println("----packetDbFieldName dbfieldName-----" + packetDbFieldName);
        //System.out.println("----packetDbFieldName dbfieldName-----" + issueDbFieldName);
        Long franchise = loginDataBean.getCompanyId();
        BasicBSONObject issueBasicBSONObject = new BasicBSONObject();
        ArrayList<String> inStockOfDepartmentList = new ArrayList<String>();
        inStockOfDepartmentList.add(loginDataBean.getDepartment() + ":D");
        ...
        List<HkLotDocument> lotDocuments = new ArrayList<>();
        lotDocuments = (List<HkLotDocument>) stockService.retrieveLotsOrPacketsForReturnForSod(issueBasicBSONObject, Boolean.FALSE, franchise);
        //System.out.println("-----retrieveLotsForIssueForSOD-----" + lotDocuments);
        List<HkPacketDocument> packetDocuments = new ArrayList<>();
        packetDocuments = (List<HkPacketDocument>) stockService.retrieveLotsOrPacketsForReturnForSod(issueBasicBSONObject, Boolean.TRUE, franchise);
        //System.out.println("-----retrievePacketsForIssueForSOD-----" + packetDocuments);
...
//        ////////////////////////////////////////////////
        ...
        issueBasicBSONObject.put(HkSystemConstantUtil.IssueStaticFieldName.IN_STOCK_OF, inStockOfValueList);
        //System.out.println("-------issueBasicBSONObject======" + issueBasicBSONObject);
        Long franchise = loginDataBean.getCompanyId();
        List<HkLotDocument> lotDocuments = new ArrayList<>();
        if (issueBasicBSONObject != null) {
            lotDocuments = (List<HkLotDocument>) stockService.retrieveLotsOrPacketsForRecieveForSod(issueBasicBSONObject, Boolean.FALSE, franchise);
            //System.out.println("-----retrieveLotsForIssueForSOD-----" + lotDocuments);
        }
        List<HkPacketDocument> packetDocuments = new ArrayList<>();
        if (issueBasicBSONObject != null) {
            packetDocuments = (List<HkPacketDocument>) stockService.retrieveLotsOrPacketsForRecieveForSod(issueBasicBSONObject, Boolean.TRUE, franchise);
            //System.out.println("-----retrievePacketsForIssueForSOD-----" + packetDocuments);
        }
        ...
        //System.out.println("-----result-----from rerout change-----" + result.toString());
    ...
    public List<SelectItem> retrieveUsersForInstockOfIssue() throws GenericDatabaseException {
        ...
        List<SelectItem> selectItemListFromUserList = centerCustomFieldTransformer.getSelectItemListFromUserList(retrieveUsers);

        return selectItemListFromUserList;
    }

    public Map<String, List<String>> issueLotsOrPackets(List<String> scannedUniqueIds, String carrierBoy) {
...
//            customFieldService.retrieveFeatureNameWithDbFieldListForFormula(feature.getId(), loginDataBean.getCompanyId(), HkSystemConstantUtil.FeatureForFormulaEvaluation.ISSUE_STOCK, HkSystemConstantUtil.CustomField.ComponentType.FORMULA);
            ...
            stockService.issueLotsOrPacketsForSod(basicBSONObject, hkIssueDocuments, dbFieldWithFormulaMap, loginDataBean.getCompanyId());
    ...
    public Map<String, List<String>> recieveLotsOrPackets(List<String> scannedUniqueIds) {
        ...
        if (missingIds.size() > 0) {
            ...
            stockService.receiveLotsOrPacketsForSod(hkIssueDocuments, franchise);
            result.put("success", null);
            return result;
        }
        return null;
    ...
    public Map<String, List<String>> returnLotsOrPackets(List<String> scannedUniqueIds) {
        ...
        if (missingIds.size() > 0) {
            ...
            stockService.returnLotsOrPacketsForSod(hkIssueDocuments, franchise);
            result.put("success", null);
            return result;
        }
