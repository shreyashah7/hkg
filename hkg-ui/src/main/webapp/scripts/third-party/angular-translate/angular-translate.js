...
angular.module('hkg.directives').factory('I18nDirectiveService', ['$resource', function(resource) {
        var rootScopePath = corsConfig.masterHkgPath + 'api/';
        var Internationalization = resource(rootScopePath + 'locale/:action',
                {},
                {
                    createLabel: {
                        method: 'POST',
...
angular.module('pascalprecht.translate').provider('$translate', [
    ...
    function($STORAGE_KEY) {
        ...
        this.$get = [
            ...
            '$rootScope',
            '$q',
            'I18nDirectiveService',
            function($log, $injector, $rootScope, $q, I18nDirectiveService) {
                ...
                var determineTranslationInstant = function(translationId, interpolateParams, interpolationId) {
                    ...
                    if (table && table.hasOwnProperty(translationId)) {
                        var translation = table[translationId];
                        ////console.log("determineTranslationInstant : Translation " + JSON.stringify(translation));
                        if (translation.substr(0, 2) === '@:') {
...
//                    console.log("Refresh Calle.=" + langKey);
                    if (!$loaderFactory) {
                ...
                $translate.removeEntity = function(originalTextWithEntity){
                    var test_str = originalTextWithEntity;
                    return test_str.substring(test_str.indexOf('.') + 1, test_str.length);
                };
                $translate.getEntity = function(originalTextWithEntity) {
                    var test_str = originalTextWithEntity;
                    return test_str.substring(0, test_str.indexOf('.') + 1);
                };
                var storedLabelList = [];
                $translate.instant = function(translationId, interpolateParams, interpolationId, originalText, entityName) {
                    if (translationId === null || angular.isUndefined(translationId)) {
                        ////console.log("1");
                        return translationId;
                    }
                    if (angular.isArray(translationId)) {
                        ////console.log("isArray : " + translationId);
                        var results = {};
                        for (var i = 0, c = translationId.length; i < c; i++) {
                            results[translationId[i]] = $translate.instant(translationId[i].replace(/\s+/g, ''), interpolateParams, interpolationId,$translate.removeEntity(translationId[i]));
                        }
                        ////console.log("2");
                        return results;
                    }
                    ...
                    if ($translationTable[possibleLangKey] && result === undefined) {
                        
                        result = originalText;
                        var listLength = Object.keys(storedLabelList).length;
                        var isDuplicate = false;
                        for (var i = 0; i < listLength; i++) {
                            if (result == storedLabelList[i]) {
                                isDuplicate = true;
                            }
                        }
                        if (!isDuplicate) {
                            storedLabelList.push(result);
                            if ($rootScope.isLabelEntryInDatabase) {
                                ...
                                I18nDirectiveService.createLabel(locales, function(res) {
                                    $translate.refresh(possibleLangKey);
                                }, function() {
                                });
                            }
                        }
...
angular.module('pascalprecht.translate').filter('translate', [
    ...
    function($parse, $translate) {
        return function(translationId, interpolateParams, interpolation) {
            ...
            return $translate.instant(translationId.replace(/\s+/g, ''), interpolateParams, interpolation, $translate.removeEntity(translationId), $translate.getEntity(translationId));
        };
    }
]);
//define(['angular', 'angularTranslate'], function() {
