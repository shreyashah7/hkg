            ...
            function ($parse, $timeout, dndDropEffectWorkaround, dndDragTypeWorkaround) {
                return function (scope, element, attr) {
                    ...
                    element.on('click', function (event) {
                        event = event.originalEvent || event;
                        scope.$apply(function () {
                            if (attr.dndSelected == 'models.selected = item') {
                                $('#renameElements').modal('show');
                            }
                ...
                return function (scope, element, attr) {
                    ...
                    element.on('drop', function (event) {
                        ...
                        if (transferredObject !== undefined) {
                            angular.forEach(targetArray, function (item, index) {
                                if (item.colField === 'column') {
                                    targetArray.splice(index, 1);
                                }
                            });
                            if (event.dataTransfer.effectAllowed === "copy") {
                                var count = 0;
                                if (transferredObject.type !== 'container') {
                                    angular.forEach(targetArray, function (item) {
                                        if (item.colField === transferredObject.colField && item.colField !== 'column') {
                                            count++;
                                            event.preventDefault();
                                        }
                                    });
                        ...
                        scope.$apply(function () {
                            if (typeof transferredObject === "object" && transferredObject.length !== undefined) {
                                //      for multiple selection drop
                                angular.forEach(transferredObject,
                                        function (obj) {
                                            targetArray.splice(getPlaceholderIndex(), 0, obj);
                                        });
                            }
                            else {
                        ...
                        if (attr.dndCallback) {
                            invokeCallbackAfterMethod(attr.dndCallback, event, transferredObject);
                        }
...
//                                : (event.offsetY || event.layerY);
                        var mousePointer = horizontal ? (getOffset(event).x)
                                : (getOffset(event).y);
                    ...
                    function invokeCallbackAfterMethod(expression, event, item) {
                        return $parse(expression)(scope, {
                            event: event,
                            index: getPlaceholderIndex(),
                            item: item || undefined,
