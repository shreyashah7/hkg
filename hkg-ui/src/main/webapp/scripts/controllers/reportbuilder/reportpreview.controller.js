define(['hkg', 'reportBuilderService', 'franchiseService', 'ngload!uiGrid', 'colorpicker.directive'], function (hkg) {
    hkg.register.controller('PreviewReport', ["$rootScope", "$scope", "$filter", "ReportBuilderService", "$sce", "$location", "$timeout", "$q", "$http", "FranchiseService",
        function ($rootScope, $scope, $filter, ReportBuilderService, $sce, $location, $timeout, $q, $http, FranchiseService) {
            $scope.subbmited = false;
            $scope.reportList = [];
            $scope.isPreview = false;
            $rootScope.mainMenu = "reportLink";
            if (localStorage.getItem("featureName") !== "null" && localStorage.getItem("featureName") !== '') {
                var name = localStorage.getItem("featureName");
                $rootScope.childMenu = name;
            }
            $rootScope.activateMenu();
            $scope.isScheduler = false;
            $scope.isExternalReport = false;
            $scope.dt = {};
            $scope.isFilter = true;
...
//                                console.log("previwDate----in paginated non filter" + JSON.stringify($scope.UIPreviewData));
            ...
            $scope.previewReport = function () {
                if (localStorage.getItem("menuReportId") !== "null" && localStorage.getItem("menuReportId") !== '') {
                    var id = localStorage.getItem("menuReportId");
                    $scope.currentPage = 1;
                    $scope.clearReportData();
                    $scope.retrieveReport(id);
                } else {
            ...
            $scope.retrieveReport = function (id) {
                ReportBuilderService.retrieveReport(id, function (response) {
                    ...
                    if ($scope.report.externalReport === true) {
                        $scope.isExternalReport = true;
                        ReportBuilderService.retrieveDashboardStatus($scope.report.id, function (res) {
                            $scope.report.isDashboard = res.data;
                        }, function (res) {
//                            alert('failed');
                        });
                        ReportBuilderService.retrieveAnalyticsCrendentials(function (response) {
                            if (response.data !== undefined && response.data !== null && response.data.hasOwnProperty('ANALYTICS_ENGINE_USERNAME')) {
                                ...
                                prom.then(function (res) {
                                    $scope.iframeUrl = $sce.trustAsResourceUrl($scope.report.query);
                                });
                            } else {
                                                    ...
                                                    angular.forEach(key.columns[0], function (data) {
                                                        if (record[data.text] !== undefined) {
                                                            if (record[data.text] != null) {
                                                                obj[data.colField] = {value:record[data.text]};
                                                            } else {
                                                                obj[data.colField] = {value:'N/A'};
                                                            }
...
//                console.log("filterForm : "+JSON.stringify(filterForm));
...
//                        $scope.tempData = res.data.records;
//                        $scope.totalItems = res.data.totalRecords;
                            $scope.isFilter = false;
                            $scope.previewData = res.data.records;
                            if ($scope.paginationOptions.pageNumber === 1) {
                                $scope.getPage();
                            } else {
                                $scope.resetGrid();
                            }
...
//            $scope.checkRowspan = function (column, index, value) {
//                if (column.isGroupBy !== undefined) {
//                    if (column.isGroupBy === true && index > 0) {
//                        return false;
//                    } else {
//                        return true;
//                    }
//                } else if (column.isGroup !== undefined) {
//                    if (column.isGroup === true && index > 0) {
//                        return false;
//                    } else {
//                        return true;
//                    }
                            ...
                            angular.forEach(filterBlock.columns, function (colorConfig) {
                                if (key === colorConfig.label && isRowSelected === false) {
                                    if ($scope.checkIsValidColorConfig(colorConfig)) {
                                        ...
                                        var isSelected = $scope.executeFilter(value, tempColorConfig);
                                        if (isSelected) {
                                            isRowSelected = true;
                                        }
                                    }
                                ...
                                angular.forEach(row, function (value, key) {
                                    if (key === colorConfig.label && isValid === true) {
                                        if ($scope.checkIsValidColorConfig(colorConfig)) {
                                            ...
                                            var isSelect = $scope.executeFilter(value, tempColorConfig);
                                            if (isSelect === false) {
                                                isValid = false;
                                            }
                                        }
...
//                console.log(JSON.stringify(colorConfig));
...
//                        }
            ...
            /* Scheduler specific code starts */
            $scope.notification = {};
            $scope.endRepeat = {};
            ...
            $scope.showSchedulerMode = function () {
//                console.log(JSON.stringify(report));
                $scope.endRepeat = {};
                $scope.notification = {};
                $scope.notification.reportId = $scope.report.id;
                ...
                ReportBuilderService.retrieveEmailConfiguration($scope.report.id, function (res) {
//                    console.log(JSON.stringify(res));
                    if (res.data === null) {
                        $scope.operationFlag = 'add';
                    } else {
                        $scope.operationFlag = 'edit';
                        $scope.notification = angular.copy(res.data);
                        if ($scope.notification.endDate !== null && $scope.notification.endDate !== undefined) {
                            $scope.notification.endDate = new Date($scope.notification.endDate);
                        }
                        $scope.setModelValuesInUI();
                    }
                }, function (res) {
                });
            };
            $scope.cancelNotification = function (notificationForm) {
                $scope.notification = {};
            ...
            $scope.clearFromRadio = function (repeatMode) {
                if ($scope.notification.endRepeatMode === "OD") {
                    ...
                    $scope.notification.endDate = null;
                    $scope.endRepeat.afterRepititionsUnits = "";
            ...
            $scope.setModelValues = function () {
                ...
                if ($scope.notificationToSave.repeatativeMode === 'W') {
                    ...
                    $scope.notificationToSave.monthlyOnDay = null;
                ...
                } else if ($scope.notificationToSave.repeatativeMode === 'M') {
                    $scope.notificationToSave.weeklyOnDays = null;
                }
            ...
            $scope.setModelValuesInUI = function () {
                //Fill weekdays value
                ...
                } else if ($scope.notification.repeatativeMode === 'D') {
                    $scope.notification.weeklyOnDays = null;
                }

                if ($scope.notification.endRepeatMode === 'AD') {
                    $scope.endRepeat.afterDaysUnits = $scope.notification.afterUnits;
                } else if ($scope.notification.endRepeatMode === 'AR') {
                    $scope.endRepeat.afterRepititionsUnits = $scope.notification.afterUnits;
                }

            };
            $scope.saveEmailConfiguration = function (form) {
                $scope.submitted = true;
                $scope.notificationToSave = angular.copy($scope.notification);
                $scope.setModelValues();
                if (form.$valid) {
                    var isAttachmentSelected = false;
                    if ($scope.notificationToSave.xlsAttachment === true || $scope.notificationToSave.pdfAttachment === true) {
                        isAttachmentSelected = true;
                    }
                    if (isAttachmentSelected) {
                        if ($scope.operationFlag == 'add') {
                            $rootScope.maskLoading();
                            ReportBuilderService.saveEmailConfiguration($scope.notificationToSave, function (data) {
                                $rootScope.unMaskLoading();
                                $scope.notification = {};
                                $scope.notificationToSave = {};
                                $scope.endRepeat = {};
                                $scope.submitted = false;
                                var msg = "Email configured successfully";
                                var type = $rootScope.success;
                                $rootScope.addMessage(msg, type);
                                $scope.operationFlag = 'add';
                                $scope.cancelNotification(form);
                                form.$setPristine();
                            }, function () {
                                $rootScope.unMaskLoading();
                                var msg = "Error while creating email configuration";
                                var type = $rootScope.failure;
                                $rootScope.addMessage(msg, type);
                            })
                        } else if ($scope.operationFlag == 'edit') {
                            $rootScope.maskLoading();
                            ReportBuilderService.saveEmailConfiguration($scope.notificationToSave, function () {
                                $rootScope.unMaskLoading();
                                $scope.notification = {};
                                $scope.notificationToSave = {};
                                $scope.endRepeat = {};
                                $scope.submitted = false;
                                var msg = "Email configuration updated successfully";
                                var type = $rootScope.success;
                                $rootScope.addMessage(msg, type);
                                $scope.operationFlag = 'add';
                                $scope.cancelNotification(form);
                                ...
                                $rootScope.unMaskLoading();
                                var msg = "Error while updating email configuration";
                                var type = $rootScope.failure;
                                $rootScope.addMessage(msg, type);
                            });
                        }
                    } else {
                        var msg = "Select atleast one attachment";
                        var type = $rootScope.failure;
                        $rootScope.addMessage(msg, type);
                    }
                } else {
                    console.log("Invalid...." + JSON.stringify(form));
                }
            };
            /* Scheduler specific code ends */

            /* Dashboard method */
            $scope.updateDashboardStatus = function () {
                var data = {};
                data.reportId = $scope.report.id;
                data.dashboardStatus = $scope.report.isDashboard;
                $rootScope.maskLoading();
                ReportBuilderService.updateDashboardStatus(data, function (res) {
                    $rootScope.unMaskLoading();
                    var msg = "Dashboard Status updated successfully";
                    var type = $rootScope.success;
//                    $rootScope.addMessage(msg, type);
                }, function (res) {
                    $rootScope.unMaskLoading();
                    var msg = "Error in updating dashboard status";
                    var type = $rootScope.failure;
                    $rootScope.addMessage(msg, type);
                });
            };
