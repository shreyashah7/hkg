define(['angular', 'dateValidate'], function() {
    globalProvider.compileProvider.directive('agDatePickerCustomSearch', ['$compile', '$filter', 'orderByFilter', '$document', function($compile, $filter, orderByFilter, $document) {
            return {
                restrict: 'E',
                template: '<div>' +
...
//                        '</a>' +
                        ...
                        '<li>' +
                        '<datepicker is-open="datePickerFlag.dateTimePicker" id="dd" ng-model="$parent.datepickerModel"  min="dateTimeMin" max="dateTimeMax" ng-change="setDate(datepickerModel,timeModel)"></datepicker>' +
                        '</li>' +
                        ...
                        '<div ng-if="isDate">' +
                        '<div class="input-group" >' +
                        '<input type="text" id="ngModel" is-open="$parent.dtPicker.dt_open"  name="ngModel" class="form-control" datepicker-popup="{{dateFormat}}"  ng-model="ngModel" min="onlyMinDate" max="onlyMaxDate" ng-change=setOnlyDate(ngModel) placeholder="{{dateFormat}}">' +
                        '<span class="input-group-btn" ng-click=(isDisabled||open($event,"dt_open"))><button ng-disabled="isDisabled" class="btn btn-default">' +
                        ...
                        '<div class="input-group" >' +
                        '<input type="text" id="fromModel" is-open="$parent.dtPicker.open_from"  name="fromModel" class="form-control" placeholder="From" datepicker-popup="{{dateRangeFormat}}"  ng-model="$parent.fromModel" max="toModel" ng-change="setFromDate(fromModel)" ng-required="ngRequired">' +
...
//                        ' <label ng-class="labelclass">{{toLabel}}</label>' +
                        ...
                        '<div class="input-group" >' +
                        '<input type="text" id="toModel" is-open="$parent.dtPicker.open_to"  name="toModel" class="form-control" min="fromModel" placeholder="To"  datepicker-popup="{{dateRangeFormat}}"  ng-model="$parent.toModel" ng-change="setToDate(toModel)" ng-required="ngRequired">' +
...
//                        'data-toggle="dropdown" data-target="#" ng-click = "datePickerFlag.toIsOpen = true;">' +
                        ...
                        '<div ng-show="toForm.toModel.$invalid" class = "error,help-block ng-hide" >' +
                        '<span class="help-block" ng-show="toForm.toModel.$error.required && !(toForm.toModel.$error.date || toForm.toModel.$error.min || toForm.toModel.$error.max)">' +
                        '{{toLabel}}{{ entity +  " not entered" | translate }}</span>' +
                        '<span class="help-block" ng-show="toForm.toModel.$error.date">{{ entity +  "Invalid date format" | translate }}</span>' +
                        '<span class="help-block" ng-show="toForm.toModel.$error.min">{{ entity +  "Violates minimum constraint" | translate }}</span>' +
                        '<span class="help-block" ng-show="toForm.toModel.$error.max">{{ entity +  "Violates maximum constraint" | translate }}</span>' +
                        '</span>' +
                        ...
                        '</div>' +
                        '</div>',
                scope: {"ngModel": "=", "fromModel": "=", "toModel": "=", "submitted": "=", "form": "=", "name": "="},
                link: function(scope, element, attrs) {
                    ...
                    if (attrs.format !== undefined) {
                        ...
                        scope.directiveFormat = "dd/MM/yyyy";
                    }
...
//                            $(element).find("#fromModel").val('');
                            ...
                            //Fill the other value for date range
                            if (attrs.type === 'daterange') {
                                if (angular.isDate(scope.fromModel) && (scope.toModel === undefined || scope.toModel === null)) {
                                    scope.toModel = new Date(scope.fromModel);
                                }
                            }
...
//                            $(element).find("#toModel").val('');
                            ...
                            //Fill the other value in case of date range
                            if (attrs.type === 'daterange') {
                                if (angular.isDate(scope.toModel) && (scope.fromModel === undefined || scope.fromModel === null)) {
                                    scope.fromModel = new Date(scope.toModel);
                                }
                            }
                            if (attrs.type === "timerange" || attrs.type === "datetimerange") {
...
//                            $(element).find("#toModel").val(getFormatedDate(value, attrs.format));
                    ...
                    if ((attrs.type && attrs.type === "date")) {
                        ...
                        var today = new Date();
                        // Set from date 1 month ago and to date as same date
                        scope.fromModel  = new Date(new Date().setMonth(today.getMonth()-1));
