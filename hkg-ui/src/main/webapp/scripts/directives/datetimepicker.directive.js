define(['angular', 'dateValidate'], function() {
    angular.module('hkg.directives').directive('agDatetimePicker', ['$compile', '$filter', function($compile, $filter) {
            return {
                restrict: 'E',
                template: '<div>' +
...
//                            $(element).find("#toModel").val(getFormatedDate(value, attrs.format));
                    ...
                    if ((attrs.type && attrs.type === "date")) {
                        ...
                        if (angular.isDefined(attrs.defaultdate) && attrs.defaultdate === "true") {
                            scope.datepickerModel = new Date();
                            scope.ngModel = getFormatedDate(new Date(), attrs.format);
                        }
                        if (attrs.ngRequired && (attrs.ngRequired === 'true' || attrs.ngRequired === true)) {
                            scope.ngRequired = true;
                        }else{
